import { test, expect } from '@playwright/test';

test.beforeEach(async ({ page }) => {

  await page.goto('https://demo.guru99.com/V1/index.php');
  await page.getByText('Bank Project').click();

  await page.locator('[name="uid"]').fill('mngr653719');
  await page.locator('[name="password"]').fill('Unajaty');
  await page.locator('[name="btnLogin"]').click();
});


test('Account No with valid input', async ({ page }) => {

  await page.locator('a[href="editAccount.php"]').click({ force: true });

  await page.locator('[name="accountno"]').fill('1234567890');
  await page.locator('[name="AccSubmit"]').click();
});


test('Account No with only characters', async ({ page }) => {

  await page.locator('a[href="editAccount.php"]').click({ force: true });

  await page.locator('[name="accountno"]').fill('bsbs');
  await page.locator('[name="AccSubmit"]').click();
  await expect(page.getByText('Characters are not allowed')).toBeVisible();
});


test('Account No with alphanumeric value', async ({ page }) => {

  await page.locator('a[href="editAccount.php"]').click({ force: true });

  await page.locator('[name="accountno"]').fill('4343jdf');
  await page.locator('[name="AccSubmit"]').click();
  await expect(page.getByText('characters are not allowed')).toBeVisible();
});


test('Account No with special characters', async ({ page }) => {

  await page.locator('a[href="editAccount.php"]').click({ force: true });

  await page.locator('[name="accountno"]').fill('@#$457852');
  await page.locator('[name="AccSubmit"]').click();

  await expect(page.getByText('Special characters are not allowed')).toBeVisible();
});


test('Submit with empty account no shows alert', async ({ page }) => {

  await page.locator('a[href="editAccount.php"]').click({ force: true });
  await page.locator('[name="AccSubmit"]').click();

  page.once('dialog', async dialog => {
    expect(dialog.message()).toContain('Please fill all fields');
    await dialog.accept();
  });

});


test('Submit with invalid characters shows alert', async ({ page }) => {

  await page.locator('a[href="editAccount.php"]').click({ force: true });

  await page.locator('[name="accountno"]').fill('gdfgch');
  await page.locator('[name="AccSubmit"]').click();

  page.once('dialog', async dialog => {
    expect(dialog.message()).toContain('Please fill all fields');
    await dialog.accept();
  });

  await expect(page.getByText('Characters are not allowed')).toBeVisible();
});

test('Reset button test', async ({ page }) => {

  await page.locator('a[href="editAccount.php"]').click({ force: true });

  await page.locator('[name="accountno"]').fill('123456');

  await page.locator('[name="res"]').click();

  await expect(page.locator('[name="accountno"]')).toHaveValue('');
});